...
export class CashierComponent implements OnInit {
  ...
  selectWorkOrder(wo: any): void {
    ...
    this.cashierService.getTransactionByWO(wo.id).subscribe({
      next: (data: any) => {
  ...
  calculateTotals(): void {
    ...
    this.subtotal = this.transaction.details.reduce((sum: number, item: any) => sum + (item.quantity * item.priceAtTransaction), 0);
    
    const net = this.subtotal - this.discount;
    const cleanNet = net > 0 ? net : 0;
    
    // PPN is 11% re-calculated reactively
    this.taxAmount = cleanNet * 0.11;
    this.grandTotal = cleanNet + this.taxAmount;

    // Automatically set default cashAmount for QRIS/Transfer
  ...
  payAndPrint(): void {
    ...
    this.cashierService.payInvoice(this.transaction.id, payload).subscribe({
      next: () => {
        ...
        // Open Print Dialog Receipt Popup
        ...
        const dialogRef = this.dialog.open(InvoicePrintDialogComponent, {
          width: '420px',
          disableClose: true,
          data: {
